(function(global){
  ...
  const TASK_RE = /\b(need|need to|have to|gotta|must|todo|to do|buy|call|email|reply|finish|clean|book|schedule|pay|pick up|drop off|check|renew|fix|send|bring|ask\s+(him|her|them|[a-z]+)|follow up)\b|\bpick\s+.{1,40}?\s+up\b|\bdrop\s+.{1,40}?\s+off\b/i;
  ...
  const TIME_RE = /\b(noon|midnight)\b|\b(\d{1,2})(?::(\d{2}))?\s*(am|pm)\b|\b(at|around|by|before|after|from|starts?)\s+(\d{1,2})(?::(\d{2}))?\b/i;
  const MULTI_RE = /\n|;|\s[-•]\s|,/i;
  ...
  function isMelt(text){ const t=clean(text); return /^melt\s*it\s*:?/i.test(t) || (t.includes(',') && t.split(',').filter(x=>clean(x).length>3).length>1) || (MULTI_RE.test(t) && t.length > 30); }
  function splitMelt(raw){
    return String(raw).replace(/^melt\s*it\s*:?/i,'')
      .split(/\n|;|\s[-•]\s|,(?!\s*\d{4}\b)/i)
      .map(s=>clean(s.replace(/^[-•]\s*/,''))).filter(Boolean);
  }
  function formatTimeParts(hour, minute, ap){
    let h = Number(hour); const min = minute || '00';
    if(ap) return `${String(h).padStart(2,'0')}:${min} ${ap.toUpperCase()}`;
  ...
  function inferTime(raw){
    ...
    let m = text.match(/\b(?:until|til|till|to|through|ending|ends? at)\s+\d{1,2}(?::\d{2})?\s*(?:am|pm)?\b/i);
    const endOnly = m && !/\b(?:at|around|from|starts?|beginning|begins?)\s+\d{1,2}|\bnoon\b|\bmidnight\b/i.test(text.replace(m[0],''));
    if(endOnly) return '';
    m = text.match(/\b(\d{1,2})(?::(\d{2}))?\s*(am|pm)\b/);
    if(m) return formatTimeParts(m[1], m[2], m[3]);
    m = text.match(/\b(?:at|around|by|from|starts?|beginning|begins?)\s+(\d{1,2})(?::(\d{2}))?\b/);
    if(m){
      const hour = Number(m[1]);
      const contextPM = /\b(lunch|dinner|supper|evening|tonight|after work|gym|practice|concert)\b/.test(text);
  ...
  function inferEndTime(raw){
    const text = clean(raw).toLowerCase();
    let m = text.match(/\b(?:until|til|till|to|through|ending|ends? at)\s+(noon|midnight)\b/i);
    if(m) return m[1].toLowerCase() === 'noon' ? '12:00 PM' : '12:00 AM';
    m = text.match(/\b(?:until|til|till|to|through|ending|ends? at)\s+(\d{1,2})(?::(\d{2}))?\s*(am|pm)?\b/i);
    if(m){
      const hour = Number(m[1]); const min = m[2] || '00'; const ap = m[3];
      if(ap) return formatTimeParts(hour, min, ap);
      const contextPM = /\b(lunch|dinner|supper|evening|tonight|after work|gym|practice|concert)\b/.test(text);
      if(contextPM && hour < 12) return `${String(hour).padStart(2,'0')}:${min} PM`;
      return `Needs review: ${hour}:${min}`;
    }
    return '';
  }
  ...
  function categoryFor(raw){
    ...
    if((hasDate(text) || hasTime(text)) && (/\b(appointment|meeting|dinner|lunch|event|practice|visit|call|dentist|doctor|therapy|pickup|pick up)\b/i.test(text) || /\bpick\s+.{1,40}?\s+up\b|\bdrop\s+.{1,40}?\s+off\b/i.test(text))) return CATEGORY.CALENDAR;
    if(IDEA_RE.test(text)) return CATEGORY.IDEAS;
    if(/\bask\s+(him|her|them|[a-z]+)|follow up\b/i.test(text)) return CATEGORY.TASKS;
  ...
  function classifySingle(raw){
    ...
    return {
      ...
      date:inferDate(text), time:inferTime(text), endTime:inferEndTime(text), createdAt:nowISO(), done:false
    };
  }
